...
program_id = 'vgsp-ising-MN3nDB5Zv6'  # program id once it is uploaded
backends = service.backends(simulator=False, min_num_qubits=4)
backend_names = [backend.name for backend in backends]
inputs = {"shots": 1024}  # Arguments passed to runtime. For POWELL: {"optimizer": POWELL()}

for backend_name in backend_names:
	try:
		options = {"backend_name": backend_name}  # Choose backend
		# Run job
